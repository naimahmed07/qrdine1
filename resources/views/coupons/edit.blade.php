...
@section('main')
    <main class="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-2xl">
        ...
        <form class="space-y-8 mb-5" action="{{ route('coupons.update', ['coupon' => $coupon]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div>
                <h3
                    ...
                    Coupon Info</h3>
                <div class="space-y-3">
                    <x-form.input label="Coupon name" type="text" name="name" id="name" value="{{ $coupon->name }}"
                        placeholder="Enter coupon name" required="1" />

                    <x-form.input label="Coupon code" type="text" name="code" id="code"
                        value="{{ $coupon->code }}" placeholder="Enter coupon code" required="1" />

                    <x-form.select label="Coupon Type" name="type" id="type" value="{{ $coupon->type }}"
                    ...
                    <x-form.input label="Coupon amount" type="text" name="value" id="value"
                        value="{{ $coupon->value }}" placeholder="Enter coupon amount" required="1" />

                    <x-form.input label="Valid from" type="date" name="valid_from" id="valid_from"
                        value="{{ $coupon->valid_from }}" placeholder="Enter start date" required="1" />

                    <x-form.input label="Valid to" type="date" name="valid_to" id="valid_to"
                        value="{{ $coupon->valid_to }}" placeholder="Enter end date" required="1" />

                    <x-form.input label="Coupon limit" type="text" name="limit" id="limit"
                        value="{{ $coupon->limit }}" placeholder="Enter coupon limit" required="1" />

                    <x-form.input-checkbox label="Active" name="active" id="active" checked="{{ $coupon->active }}"
                        required="0" />
                </div>
            </div>

            <x-button type="submit" id="update-coupon-btn" usage="update"
